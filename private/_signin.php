...
if($_SERVER['REQUEST_METHOD'] == 'POST' && $_POST['login_user_email'] !="" && $_POST['login_user_password'] != ""){
    ...
    $sql = "SELECT * FROM `login` WHERE login_user_email='$login_user_email'";
    ...
    if ($num != 0) {
        ...
        $hash = password_hash($login_user_password,PASSWORD_DEFAULT);

        // Adding data to server
        $sql = "INSERT INTO `login` (`login_user_email`, `login_user_password`, `login_user_date_time`) VALUES ('$login_user_email', '$hash', current_timestamp())";
        $result = mysqli_query($conn,$sql);
        if (!$result) {
                    ...
                    <form class="" action="<?php $_SERVER['PHP_SELF']?>" method="POST">
                        <div class="form-floating mb-3">
                            <input type="text" class="form-control rounded-3" id="floatingInput"
                                placeholder="name@example.com" name="login_user_email">
                            <label for="floatingInput">Email</label>
                        </div>
                        <div class="form-floating mb-3">
                            <input type="password" class="form-control rounded-3" id="floatingPassword"
                                placeholder="Password" name="login_user_password">
